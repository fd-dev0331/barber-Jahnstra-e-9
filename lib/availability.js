...
import { closureOn, closuresBetween } from './holidays.js';

/**
 * @returns {Promise<{slots: Array<{start:string,end:string,available:boolean}>, closed:boolean, closure?:object}>}
 * `closure` kann vorab übergeben werden (null = offen), sonst wird er abgefragt.
 */
export async function getAvailability({ business, employee, service, date, closure: knownClosure }) {
  // Feiertag oder Schließtag: gar keine Zeiten, auch keine belegten.
  const closure = knownClosure !== undefined ? knownClosure : await closureOn(business.id, date);
  if (closure) return { slots: [], closed: true, closure };
...
export async function findNextAvailableDate({ business, employee, service, fromDate, maxDays = 14 }) {
  // Schließtage einmal für den ganzen Zeitraum laden statt je Tag.
  const closures = await closuresBetween(business.id, addDays(fromDate, 1), addDays(fromDate, maxDays));
  for (let i = 1; i <= maxDays; i += 1) {
    const date = addDays(fromDate, i);
    if (closures.has(date)) continue;
    const { slots } = await getAvailability({ business, employee, service, date, closure: null });
...
export async function assertSlotFree({
  ...
  if (!ignoreWorkingHours) {
    // Feiertag oder Schließtag: nicht buchbar. Nur die Leitung darf das bewusst übergehen.
    if (await closureOn(business.id, date)) return false;
