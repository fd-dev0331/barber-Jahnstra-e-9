...
async function run() {
  try {
    ...
    if (!['23505', '42P07', '42701', '40P01'].includes(err?.code)) throw err;
    await query(SCHEMA_EXTENSIONS);
  }
  /* Danach die Zugriffssperre für die Supabase Data API (lib/security.js).
     Hier und nicht nur in db:migrate: Tabellen, die erst zur Laufzeit entstehen,
     brauchen RLS genauso. Scheitert sie, bleibt die Website erreichbar. */
