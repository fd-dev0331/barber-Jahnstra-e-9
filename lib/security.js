/* Schutz der Datenbank vor direktem Zugriff über die Supabase Data API.

   Ausgangslage (Supabase Security Advisor: „Table publicly accessible"):
   Supabase gibt den Rollen `anon` und `authenticated` standardmäßig alle Rechte
   auf jede Tabelle im Schema public — auch auf künftige. Der anon-Key ist per
   Design öffentlich; ohne Row Level Security konnte damit jeder über
   https://<projekt>.supabase.co/rest/v1/ Passwort-Hashes, Sitzungen,
   Kundendaten und Termine lesen, ändern und löschen. `authenticated` ist nicht
   harmloser: über die offene Supabase-Auth-Registrierung bekommt jeder mit dem
   anon-Key auch diese Rolle.

   Tatsächlicher Zugriffsweg dieses Projekts (geprüft im Code):
     Browser (Website, Verwaltung, Telegram Mini App)
       → nur /api/*  (Vercel Functions; kein supabase-js, kein anon-Key im Frontend)
       → lib/db.js   (pg, direkte Verbindung über DATABASE_URL bzw. POSTGRES_URL)
       → PostgreSQL als Eigentümer der Tabellen
   Kein Client braucht die Data API. Deshalb bekommen `anon` und
   `authenticated` auf keiner Tabelle ein Recht und keine erlaubende Policy.

   Was diese Migration tut — idempotent, ohne Daten oder Tabellen anzufassen:
     1. RLS einschalten, auf jeder Tabelle in public, die der ausführenden Rolle
        gehört. Der Eigentümer ist von RLS ausgenommen (kein FORCE), das Backend
        arbeitet also unverändert weiter.
     2. `anon` und `authenticated` (und PUBLIC) alle Tabellen-, Spalten- und
        Sequenzrechte entziehen — auch auf Views.
     3. Je Tabelle eine RESTRICTIVE-Policy `deny_data_api` für diese beiden
        Rollen mit USING (false). Solange es keine erlaubende Policy gibt, sperrt
        RLS ohnehin; die restriktive Policy sorgt dafür, dass auch eine später
        versehentlich angelegte `USING (true)`-Policy nichts öffnet.
     4. Die Default Privileges der ausführenden Rolle so ändern, dass künftige
        Tabellen `anon`/`authenticated` nicht wieder automatisch bekommen.

   Sicherheitsnetz gegen das Aussperren des Backends: eine Tabelle, die nicht der
   ausführenden Rolle gehört, wird übersprungen (mit WARNING). Nur Eigentümer
   dürfen RLS schalten, und nur für sie ist garantiert, dass RLS sie nicht
   selbst trifft. Eine Tabelle mit FORCE ROW LEVEL SECURITY wird nie berührt.

   Was sich nicht ändert: `service_role` (serverseitiger Schlüssel mit
   BYPASSRLS, im Projekt nicht benutzt), Funktionen, Erweiterungen, Schema-
   Rechte (USAGE). Details und Begründung: README.md, Abschnitt Datenbank-Sicherheit.

   Dieselbe SQL steht in db/migrations/001_secure_rls.sql;
...
/**
 * Migration ausführen. Liefert die Meldungen (NOTICE/WARNING) zurück.
 * Wirft bei SQL-Fehlern; ensureSecurity() fängt das für den Laufzeitbetrieb ab.
 */
export async function applySecurity(pool = getPool()) {
  const client = await pool.connect();
  const messages = [];
  const onNotice = (msg) => messages.push({ severity: msg.severity, message: msg.message });
  client.on('notice', onNotice);
  try {
    await client.query(SECURITY_SQL);
  } finally {
...
/** Nach einem Fehler nicht bei jeder Anfrage erneut versuchen. */
...
/**
 * Einmal pro Function-Instanz, nach den Schema-Ergänzungen (lib/schema.js):
 * so sind auch Tabellen abgedeckt, die erst zur Laufzeit entstehen.
 *
 * Ein Fehler hier legt die Website nicht lahm — er wird laut protokolliert und
 * nach RETRY_AFTER_MS erneut versucht. Im Normalbetrieb ist alles schon
 * erledigt; dann liest die Abfrage nur den Katalog und nimmt keine Sperren.
 */
export function ensureSecurity() {
  if (pending) return pending;
  if (failedAt && Date.now() - failedAt < RETRY_AFTER_MS) return Promise.resolve();
  pending = applySecurity()
    .then((messages) => {
      failedAt = 0;
      for (const { severity, message } of messages) {
        if (severity === 'WARNING') console.error(`[db-security] ${message}`);
        else console.log(`[db-security] ${message}`);
      }
    })
    .catch((err) => {
      console.error('[db-security] Migration fehlgeschlagen:', err?.message ?? err);
      failedAt = Date.now();
      pending = null;
    });
  return pending;
