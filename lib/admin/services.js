...
import { text, integer, oneOf, bool, uuid, slugify, ValidationError } from './util.js';
...
export async function list(req, res, user) {
  const { rows } = await query(
    `SELECT s.id, s.slug, s.name, s.description, s.is_offer, s.duration_minutes,
      ...
      ORDER BY s.status, s.is_offer DESC, s.sort_order, s.name`,
    [user.businessId]
  );
...
export async function create(req, res, body, user) {
  ...
  const isOffer = bool(body.isOffer, false);
  ...
  try {
    const { rows } = await query(
      `INSERT INTO service (business_id, slug, name, description, is_offer,
                            duration_minutes, price_cents, sort_order, status)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'ACTIVE')
       RETURNING id, slug, name, description, is_offer, duration_minutes, price_cents, status, sort_order`,
      [user.businessId, slug, name, description, isOffer, duration, price, sortOrder]
    );
...
export async function update(req, res, body, user, id) {
  ...
  if (body.isOffer !== undefined) push('is_offer', bool(body.isOffer, false));
