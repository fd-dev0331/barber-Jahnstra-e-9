...
import { isValidDateString, addDays, dateInZone } from '../time.js';
import { closureOn } from '../holidays.js';
...
export async function overview(req, res, user) {
  ...
  /* Ein Mitarbeiter sieht auch im Dashboard nur die eigenen Termine — dieselbe
     Regel wie in list(). Vorher lieferte dieser Endpunkt einem EMPLOYEE die
     Termine aller Kolleginnen und Kollegen (info.md §22). */
  const ownUserId = user.role === 'EMPLOYEE' ? user.id : null;
  const ownOnly = (param) =>
    `AND ($${param}::uuid IS NULL OR b.employee_id IN (SELECT id FROM employee WHERE user_id = $${param}::uuid))`;
  ...
  const [{ rows: counts }, { rows: today }, { rows: upcoming }, { rows: integration }] = await Promise.all([
    query(
      `SELECT (SELECT count(*) FROM employee WHERE business_id = $1 AND status = 'ACTIVE')::int AS employees,
              ...
              (SELECT count(*) FROM booking b WHERE b.business_id = $1 AND b.status IN ('PENDING','CONFIRMED')
                 AND b.start_time >= now() ${ownOnly(2)})::int AS upcoming_bookings`,
      [business.id, ownUserId]
    ),
    query(
      ...
      [business.id, business.timezone, ownUserId]
    ),
    query(
  ...
  json(res, 200, {
    ...
    // Konto-Adresse und Fehlertext sind Sache der Leitung; ein EMPLOYEE sieht nur den Zustand.
...
export async function create(req, res, body, user) {
  ...
  // Feiertag/Schließtag gilt auch hier; "außerhalb der Arbeitszeit" hebt ihn bewusst auf.
  if (!outsideHours) {
    const closure = await closureOn(business.id, dateInZone(start, business.timezone));
    if (closure) {
      return fail(res, 409, 'closed_day', `An diesem Tag ist der Salon geschlossen (${closure.name}).`);
    }
  }
...
/**
 * DELETE /api/admin/bookings/:id — einen stornierten Termin endgültig löschen.
 *
 * Nur stornierte. Ein wahrgenommener, offener oder als „nicht erschienen"
 * vermerkter Termin gehört zur Geschichte des Betriebs und bleibt bestehen; wer
 * ihn loswerden will, storniert ihn zuerst und trifft damit eine sichtbare
 * Entscheidung. Gelöscht wird die Zeile wirklich — das ist nicht umkehrbar,
 * deshalb fragt die Oberfläche vorher nach.
 */
export async function remove(req, res, user, id) {
  ...
  const { rows } = await query(
    `SELECT b.id, b.status, b.google_event_id, e.google_calendar_id
       FROM booking b JOIN employee e ON e.id = b.employee_id
      WHERE b.id = $1 AND b.business_id = $2`,
    [id, business.id]
  );
  ...
  if (booking.status !== 'CANCELLED') {
    return fail(res, 409, 'not_cancelled',
      'Nur stornierte Termine lassen sich löschen. Storniere den Termin zuerst.');
  }

  /* Beim Stornieren wird der Kalendereintrag gelöscht und die ID geleert. Bleibt
     hier doch eine stehen, war Google damals nicht erreichbar — dann jetzt der
     zweite Versuch, aber ohne das Löschen daran scheitern zu lassen. */
  let warning;
  if (booking.google_event_id && booking.google_calendar_id) {
    try {
      ...
      if (!(err instanceof GoogleUnavailableError)) throw err;
      warning = 'Der Termin ist gelöscht, der Eintrag im Google Kalender konnte aber nicht entfernt werden. Bitte dort nachsehen.';
    }
  }

  await query('DELETE FROM booking WHERE id = $1 AND business_id = $2', [id, business.id]);
  json(res, 200, { ok: true, ...(warning ? { warning } : {}) });
}
