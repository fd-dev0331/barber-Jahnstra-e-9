...
import { text, uuid, integer, oneOf, bool, timeOfDay, isoDateTime, ValidationError } from './util.js';
...
const SELECT = `
  SELECT e.id, e.name, e.role_label, e.status, e.google_calendar_id, e.sort_order, e.user_id,
         e.show_on_website, e.headline, e.bio, e.languages, e.experience_years, e.photo_media_id,
...
function shape(row, hours = [], absences = []) {
  return {
    ...
    // Öffentliches Profil für den Team-Slider auf der Startseite.
...
/** Mehrzeiliger Text (Profiltext): Absätze bleiben, Steuerzeichen nicht. */
function multiline(value, { max, field }) {
  ...
  if (cleaned.length > max) throw new ValidationError('invalid_input', `${field} ist zu lang (höchstens ${max} Zeichen).`);
  return cleaned;
}

/** Profilfelder aus dem Body — nur die, die mitgeschickt wurden. */
async function profileFields(body, businessId) {
  const fields = [];
  if (body.showOnWebsite !== undefined) fields.push(['show_on_website', bool(body.showOnWebsite, true)]);
  if (body.headline !== undefined) fields.push(['headline', text(body.headline, { max: 120, field: 'Überschrift' })]);
  if (body.bio !== undefined) fields.push(['bio', multiline(body.bio, { max: 2000, field: 'Profiltext' })]);
  if (body.languages !== undefined) fields.push(['languages', text(body.languages, { max: 120, field: 'Sprachen' })]);
  if (body.experienceYears !== undefined) {
    fields.push(['experience_years', integer(body.experienceYears, { min: 0, max: 80, field: 'Erfahrung' })]);
  }
  if (body.photoMediaId !== undefined) {
    const photo = body.photoMediaId ? uuid(body.photoMediaId, { field: 'Foto' }) : null;
    if (photo) await assertMedia(businessId, photo);
    fields.push(['photo_media_id', photo]);
  }
  return fields;
}
...
export async function create(req, res, body, user) {
  ...
  const profile = await profileFields(body, user.businessId);

  const employee = await withTransaction(async (client) => {
    const columns = ['business_id', 'name', 'role_label', 'google_calendar_id', 'sort_order', 'status', ...profile.map(([c]) => c)];
    const values = [user.businessId, name, roleLabel, calendarId, sortOrder, 'ACTIVE', ...profile.map(([, v]) => v)];
...
export async function update(req, res, body, user, id) {
  ...
  const { rows: existing } = await query(
    'SELECT id, photo_media_id FROM employee WHERE id = $1 AND business_id = $2', [id, user.businessId]
  );
  if (!existing.length) return fail(res, 404, 'not_found', 'Diesen Mitarbeiter gibt es nicht.');
  const previousPhoto = existing[0].photo_media_id;
  ...
  for (const [column, value] of await profileFields(body, user.businessId)) push(column, value);
  ...
  // Ein ersetztes oder entferntes Foto nicht als Leiche in der Datenbank lassen.
  if (body.photoMediaId !== undefined && previousPhoto && previousPhoto !== body.photoMediaId) {
    await deleteIfUnused(user.businessId, previousPhoto);
  }
...
export async function remove(req, res, user, id) {
  ...
  const { rows: deleted } = await query(
    'DELETE FROM employee WHERE id = $1 AND business_id = $2 RETURNING photo_media_id', [id, user.businessId]
  );
  await deleteIfUnused(user.businessId, deleted[0]?.photo_media_id);
